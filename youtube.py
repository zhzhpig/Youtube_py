...
def check_for_updates():
    ...
    if app_version != latest_version:
        logger.info(f"最新版本为: {latest_version},当前版本为：{app_version}")
        # 显示弹窗询问用户是否继续
    ...
    try:
        ...
        with open(file_path, mode) as f:
            ...
            for data in response.iter_content(chunk_size=chunk_size):
                ...
                if elapsed_time >= 1:  # 每秒更新一次下载速度
                    init_speed = (downloaded - last_downloaded) / elapsed_time
                    ...
                    eta_seconds = remaining_bytes / init_speed if init_speed > 0 else 0
                    hours, remainder = divmod(int(eta_seconds), 3600)  # 3600 秒为一小时
                    minutes, seconds = divmod(remainder, 60)
                    eta_str = f"{hours:02d}:{minutes:02d}:{seconds:02d}"
                    progress_window.title(f"更新下载中 ({speed:.2f} {speed_units} 剩余时间: {eta_str})")
...
def download_update(version,url):
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 206:
            ...
            progress_window.geometry("500x50")
            progress_window.resizable(False, False)
            progress_window.iconbitmap(icon_path)
            
            # 创建进度条
            progress_bar = ttk.Progressbar(progress_window, orient="horizontal", length=480, mode="determinate")
            progress_bar.pack(pady=10)
            
            # 创建标签显示文件名
            label = tk.Label(progress_window, text=f"正在下载: {version}")
            label.pack(pady=5)
            ...
            error_log.append(f"请求更新下载地址{url}失败，错误为{response.status_code}")
    except Exception as e:
